...
public class LoadingLayout extends FrameLayout {
	...
	private boolean mIsHeaderImgVisible = true;
	...
	public LoadingLayout(Context context, final Mode mode, TypedArray attrs) {
		...
		this.mEmptyImage = (ImageView) header.findViewById(R.id.empty_image);
		...
		if (attrs != null && attrs.hasValue(R.styleable.PullToRefresh_ptrHeaderTextColor)) {
			ColorStateList colors = attrs.getColorStateList(R.styleable.PullToRefresh_ptrHeaderTextColor);
			setTextColor(null != colors ? colors : ColorStateList.valueOf(0xFF000000));
		}
		if (attrs != null && attrs.hasValue(R.styleable.PullToRefresh_ptrHeaderSubTextColor)) {
			ColorStateList colors = attrs.getColorStateList(R.styleable.PullToRefresh_ptrHeaderSubTextColor);
			setSubTextColor(null != colors ? colors : ColorStateList.valueOf(0xFF000000));
		}
		if (attrs != null && attrs.hasValue(R.styleable.PullToRefresh_ptrHeaderBackground)) {
		...
		if (attrs != null && attrs.hasValue(R.styleable.PullToRefresh_ptrDrawable)) {
	...
	public void setHeaderImgVisible(boolean isVisible) {
		if (isVisible) {
			mHeaderImage.setVisibility(View.VISIBLE);
		} else {
			mHeaderImage.setVisibility(View.GONE);
		}
		mIsHeaderImgVisible = isVisible;
		reset();
	}

	public void setEmpty(String aTextStr) {
		mHeaderImage.setVisibility(View.GONE);

		mHeaderImage.clearAnimation();

		this.mEmptyImage.setVisibility(View.GONE);
		this.mHeaderText.setText(aTextStr);
	}

	public void setEmpty(int aResId) {
		mHeaderImage.setVisibility(View.GONE);
		...
		this.mEmptyImage.setImageResource(aResId);
	}
