...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout {
	...
	public interface OnPullDownListener {
		 ...
		 * 滑动时回调
		 * */
		void doLayoutScroll(float height);
	...
	public final boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE: {
			if (mIsBeingDragged) {
				...
				pullEvent();
				if (getCurrentMode() == Mode.PULL_DOWN_TO_REFRESH && pullDownListener != null) {
					pullDownListener.doLayoutScroll(mLastMotionY);
				}
				return true;
			}
			break;
		...
		case MotionEvent.ACTION_UP: {
			if (getCurrentMode() == Mode.PULL_DOWN_TO_REFRESH && pullDownListener != null) {
				pullDownListener.onfinishScroll();
			}
			if (mIsBeingDragged) {
				mIsBeingDragged = false;
	...
	protected final void setHeaderScroll(int y) {
		int minHeight = -AppUtil.dip2px(getContext(), 50);
		if (y < minHeight) {
			y = minHeight;
		}
		scrollTo(0, y);
	...
	public void setAutoLoadMore() {
		...
		// // smoothScrollTo(0);
		// smoothScrollTo(mHeaderHeight/2);
		// this.mFooterLayout.reset();
		// this.mFooterLayout.setVisibility(View.GONE);
		// return;
		// }
		//
		// this.mFooterLayout.setVisibility(View.VISIBLE);
	...
	public void setIsPullDown(boolean isPullDowning) {
		this.isPullDown = isPullDowning;
	}
	...
	public float getLastMoveHeight() {
		return mLastMotionY;
	}

	public void setOnPullRefresh(OnPullDownListener pullDownListener) {
