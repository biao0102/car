...
public class UpdateLocationResponse extends FlResponseBase {

	public ArrayList<Locations> mLocations = new ArrayList<Locations>();

	public UpdateLocationResponse(DataCollection dataSource) {
		super(dataSource);
	}
	...
	private void praseBusinessList(JSONArray aList) {
		for (int i = 0; i < aList.length(); i++) {
			try {
				...
				aLocations.setCityId(aData.getString("cityId"));
				aLocations.setCityName(aData.getString("cityName"));
				mLocations.add(aLocations);
			} catch (JSONException e) {
				e.printStackTrace();
			}
		}
