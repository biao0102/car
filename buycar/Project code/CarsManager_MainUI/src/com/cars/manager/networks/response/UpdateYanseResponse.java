...
public class UpdateYanseResponse extends FlResponseBase {

	public ArrayList<Yanse> mYanses = new ArrayList<Yanse>();

	public UpdateYanseResponse(DataCollection dataSource) {
		super(dataSource);
	}
	...
	private void praseBusinessList(JSONArray aList) {
		for (int i = 0; i < aList.length(); i++) {
			try {
				...
				Yanse aYanse = new Yanse();
				aYanse.setColor(aData.getString("color"));
				aYanse.setColorName(aData.getString("colorName"));
				mYanses.add(aYanse);
			} catch (JSONException e) {
				e.printStackTrace();
			}
