...
import com.cars.manager.utils.imageChooser.api.ChosenVideo;
...
public class VideoProcessorThread extends MediaProcessorThread {
	...
	private String createPreviewImage() throws IOException {
		...
		if (bitmap != null) {
			previewImage = FileUtils.getDirectory(foldername) + File.separator + Calendar.getInstance().getTimeInMillis() + ".jpg";
	...
	private String createThumbnailOfVideo() throws IOException {
		...
		if (bitmap != null) {
			thumbnailPath = FileUtils.getDirectory(foldername) + File.separator + Calendar.getInstance().getTimeInMillis() + ".jpg";
