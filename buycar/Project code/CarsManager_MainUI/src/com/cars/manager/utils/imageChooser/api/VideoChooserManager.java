...
import com.cars.manager.utils.imageChooser.api.config.Config;
import com.cars.manager.utils.imageChooser.threads.VideoProcessorListener;
import com.cars.manager.utils.imageChooser.threads.VideoProcessorThread;
...
public class VideoChooserManager extends BChooser implements VideoProcessorListener {
	...
	public VideoChooserManager(android.app.Fragment fragment, int type, boolean shouldCreateThumbnails) {
		super(fragment, type, DIRECTORY, shouldCreateThumbnails);
	}

	public VideoChooserManager(Activity activity, int type, String foldername, boolean shouldCreateThumbnails) {
		super(activity, type, foldername, shouldCreateThumbnails);
	}

	public VideoChooserManager(Fragment fragment, int type, String foldername, boolean shouldCreateThumbnails) {
		super(fragment, type, foldername, shouldCreateThumbnails);
	}

	public VideoChooserManager(android.app.Fragment fragment, int type, String foldername, boolean shouldCreateThumbnails) {
		super(fragment, type, foldername, shouldCreateThumbnails);
	}
	...
	private String captureVideoCurrent() throws Exception {
		...
		try {
			...
			filePathOriginal = FileUtils.getDirectory(foldername) + File.separator + Calendar.getInstance().getTimeInMillis() + ".mp4";
